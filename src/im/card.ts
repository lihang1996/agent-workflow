...
/** 合并窗口内多次 push，只提交最新卡；finish 强制写出终态。 */
export class ThrottledCardUpdater {
  ...
  constructor(
    ...
    /** 运行中频繁 patch 会与按钮回调打架；默认 2.5s 合并一次。 */
  ...
  async finish(finalCard: CardJson): Promise<void> {
    ...
    // final: 绕过「已终态则丢弃进度 patch」的守卫，确保成功/失败卡能写出。
    await this.updateCard(finalCard, { final: true });
  }
  ...
  async cancel(): Promise<void> {
    ...
    this.timer = undefined;
    this.pendingCard = undefined;
    // 不 await 在途 patch：卡片回调必须在 3 秒内返回，阻塞会导致客户端报错。
  }
