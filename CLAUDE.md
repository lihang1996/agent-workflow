...
- QA/评审 CLI 进程死在 `RetriableError: [unavailable] PING timed out` / `[resource_exhausted]` 时，旧逻辑把任意质量步骤失败默认移交开发；开发只重写门禁时间戳、评审再过、QA 再撞配额 → 死循环。正确做法：编排层故障分类为 `orchestration`，停在当前步骤等人重试，不要记 `quality_fix_request`；已误移交的 `/workflow retry` 要把游标拉回来源质量步骤。
...
- 架构师 CLI `result=done` 且 design gate `pass`，飞书却弹出「补齐 Gate/artifact 证据」：`[RESULT:done][GATE_RESULT]{...sha256...}` 粘在同一行时，`]` 后整段 JSON 被收成 done 的 reason；`shouldPauseAsEvidenceBlock` 又用全文去匹配光秃秃的 `sha256`。正确做法：RESULT 同行的 GATE_RESULT/DECISION 等协议标记不是人类原因；`[RESULT:done]` 不得再走证据假暂停；`sha256` 只在「不匹配/错误」语境才算 gate-evidence。改完必须重启 `pnpm start` 再点原阻塞卡重试，不要终止流水线。
