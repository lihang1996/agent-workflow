...
- 确认后点击「发布到飞书云文档」，系统会创建 Docx 并写入 Markdown 内容；应用需要具备 Docx 创建/编辑权限。
- 评审卡的“要求修改”会同步为该云文档的全文评论，并把修改意见交给产品经理；修订版会重新进入确认流程。
...
- **云文档发布/评论失败**：检查飞书应用是否已申请并获批 Docx 创建/编辑、Drive 文件评论权限，并确认应用对目标文档有访问权。

## 已知限制 / 待补充

- 当前仓库未提供自动化测试脚本；可补充适配器、会话状态机和消息解析的单元测试。
- 飞书应用创建、权限清单、生产进程托管和日志/监控方案需要补充部署章节。
- 云文档外部用户直接新增的评论暂未通过事件订阅自动拉取；当前评审卡提交的意见会同步到云文档并自动交给产品经理处理。
